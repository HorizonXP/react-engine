...
var extend = require('lodash-node/compat/object/extend');

// safely require the peer-dependencies
var React = util.safeRequire('react');
var ReactDOMServer = require('react-dom/server');
var createLocation = util.safeRequire('history/lib/createLocation');
var RoutingContext = util.safeRequire('react-router').RoutingContext;
var match = util.safeRequire('react-router').match;
  ...
  return function render(thing, options, callback) {
    ...
    try {
      if (this.useRouter) {
        ...
        createOptions.location = createLocation(thing);
        ...
        // });
        match({ createOptions.routes, createOptions.location }, (error, redirectLocation, renderProps) => {
          if (redirectLocation) {
            // redirection
            return done(redirectLocation);
          } else if (error) {
            throw(error);
            ...
            var myCreateElement = (Component, props) => {
              return React.createElement(Component, extend(props, data));
            };
            renderProps['createElement'] = myCreateElement;
            componentInstance = React.createElement(RoutingContext, renderProps);
          }
        })
        Router.run(createOptions.routes, createOptions.location, (error, initialState, transition) => {
          if (error === null) {
          } else if (error === undefined && transition.isCancelled && transition.redirectInfo && !transition.abortReason) {
            // redirection
            return done(trans
            throw(transition);
          } else {
